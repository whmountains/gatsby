...
`gatsby-link` is a wrapper for the `<Link>` component in react-router.  It automatically prefixes urls and handles prefetching on browsers that don't support ServiceWorker.  It's not included with Gatsby, so you should install it:

`npm install --save gatsby-link`

`gatsby-link` auto-detects whether to use a plain `<Link>` or `<NavLink>` based on what props you pass it.  This is react-router v4 so the `<IndexLink>` component is no longer used:
